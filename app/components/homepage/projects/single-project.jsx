...
import school from '/svg/school/logo.png';

const SingleProject = ({ project = {} }) => {
  // 프로젝트 데이터가 없을 경우 기본값 설정
  const {
    name = 'Project Name',
    ...
    code = '',
    demo = '',
    image = school, // 이미지가 없을 경우 placeholder 사용
    features = [],
  } = project;
  console.log(project);
  ...
  return (
    <div className='group w-full h-fit flex flex-col items-center justify-center relative cursor-text overflow-hidden px-3 md:px-8 py-[1.4rem] bg-[linear-gradient(90deg,#281e57_0%,#201435_100%)] shadow-2xl rounded-lg border border-[#1a1443]'>
      ...
      <div className='flex flex-col items-center justify-between w-full h-full'>
        ...
        <div className="p-6">
          <Image
            src={image.src || placeholder.src} // 이미지가 있으면 표시하고, 없으면 placeholder 표시
            alt={name}
            width={1080}
            height={720}
